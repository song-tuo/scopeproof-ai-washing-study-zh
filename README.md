...
- **回响自动确认：** 基础网址先收集回响用户编号，Supabase 自动平衡分组；第 12 题写入云端后自动返回回响数据，不要求复制完成码。
- **理解练习：** 正式答题前完成两道生活化例题，分别辨认“缺少测试”和“同样条件下结果相反”；两题都答对后才能开始。
...
本机的 `localhost`、`127.0.0.1` 和 `::1` 会自动进入预览模式，不写入 Supabase；`preview=1` 仍可用于正式域名上的研究者检查。
...
正式招募、数据导出和版本冻结步骤见 `RESEARCHER_GUIDE_ZH.md`；H3 的模型与计分口径见 `H3_MEASUREMENT_V10_ZH.md`。
